...
3. Kad projekat bude spreman: **Project Settings** → **API**:
   ...
   - **service_role** key → to je `SUPABASE_SERVICE_ROLE_KEY` (potreban da admin može da kreira nove predavače; nikad ga ne izlaži u frontendu)
...
2. U `.env.local` unesi: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, i (za admin „Novi predavač”) `SUPABASE_SERVICE_ROLE_KEY` iz Supabase → Project Settings → API.
3. Pokretanje:
   ```bash
   npm install
   npm run dev
...
3. U **Environment Variables** dodaj:
   ...
   - `SUPABASE_SERVICE_ROLE_KEY` = tvoj Supabase service_role key (da admin na produkciji može da kreira predavače)
